...
const WishlistModal = ({ isOpen, onClose, onSave, item }) => {
   ...
   useEffect(() => {
      if (item) {
         ...
         if (item.imageUrls) {
            validateImageUrl(item.imageUrls);
         }
      } else {
         resetForm();
      }
   ...
   // Updated function to validate image URL format, including data URIs
   const validateImageUrl = (url) => {
      if (!url) {
         setIsValidImageUrl(true); // Empty is valid since it's optional
         setImageError("");
         return true;
      }
   ...
   const handleSubmit = async (e) => {
      ...
      if (imageUrls && !validateImageUrl(imageUrls)) {
