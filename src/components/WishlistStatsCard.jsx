    ...
    return (
        <div className={`border-4 border-black rounded-xl p-4 sm:p-5 ${bgColor} flex-1 relative shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden transform transition-all hover:-translate-y-1`}>
            ...
            <div className="bg-white border-3 border-black rounded-lg p-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)] relative z-10">
                <p className={`text-center ${isPrice ? "text-2xl sm:text-xl md:text-2xl lg:text-3xl" : "text-3xl sm:text-2xl md:text-3xl lg:text-4xl"} font-black`}>
                    {isPrice ? `Rp ${value.toLocaleString()}` : value}
                </p>
            </div>

            {/* Subtle corner decoration */}
            <div className="absolute top-0 right-0 w-16 h-16 bg-black/5 -mt-8 -mr-8 transform rotate-45"></div>
        </div>
    );
};
